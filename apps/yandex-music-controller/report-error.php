<?php
/**
 * Public endpoint for reporting plugin errors
 *
 * Usage: POST /apps/yandex-music-controller/report-error.php
 * Body: JSON with error data
 */
...
require_once __DIR__ . '/lib/ErrorTracker.php';
...
try {
    ...
    // Check rate limit (moderate limit for errors)
    if ($config['rate_limit']['enabled']) {
        $rateLimiter = new RateLimiter(
            $db,
            $config['rate_limit']['error_max_requests'],
        ...
        if (!$rateLimiter->check($ipAddress, 'error')) {
            sendResponse(
                false,
        ...
        $rateLimiter->increment($ipAddress, 'error');
    }
    ...
    try {
        $errorTracker->track($errorData);
        sendResponse(true, null, 201);
    } catch (InvalidArgumentException $e) {
        sendResponse(false, 'Invalid error data', 400);
    }
